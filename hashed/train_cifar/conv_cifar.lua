...
-- {{{
  ...
  cmd:option('-nhu', 1024, 'number of hidden units')
  cmd:option('-ncLayers', 5, 'total number of conv layers, excluding input and output layers')
...
-- {{{
  ...
  local function conv_module(inSize, outSize, kerWidth, kerHeight, padWidth, padHeight)
    local conv = nil
    if hash_config then
      conv = nn.HashConvMM(inSize, outSize, kerHeight, kerWidth, 1, 1, padWidth, padHeight, hash_config)
    else
      conv = nn.SpatialConvolutionMM(inSize, outSize, kerWidth, kerHeight, 1, 1, padWidth, padHeight)
    end
    return conv
  end

  local dcLayers = {3, 64, 128, 256, 512, 512}
  local kerSize = 5
  local padSize = (kerSize - 1) / 2
  local poolSize = 2
  local image_color = dcLayers[1]
  local image_size = 32
  -- model
  local model = nn.Sequential()
  model:add(nn.Reshape(image_color, image_size, image_size))

  -- model:add(conv_module(3, 64, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(conv_module(64, 64, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(nn.SpatialMaxPooling(2, 2))
  --
  -- model:add(conv_module(64, 128, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(conv_module(128, 128, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(nn.SpatialMaxPooling(2, 2))
  --
  -- model:add(conv_module(128, 256, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(conv_module(256, 256, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(conv_module(256, 256, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(nn.SpatialMaxPooling(2, 2))
  --
  -- model:add(conv_module(256, 512, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(conv_module(512, 512, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(conv_module(512, 512, 3, 3, 1, 1))
  -- model:add(nn.ReLU())
  -- model:add(nn.SpatialMaxPooling(2, 2))
  ...
  --
  -- model:add(linear_module(512, 512))
  -- model:add(nn.ReLU())
  --
  -- model:add(linear_module(512, 512))
  -- model:add(nn.ReLU())
  --
  -- model:add(linear_module(512, 10))

  for l = 1, opt.ncLayers do
    model:add(conv_module(dcLayers[l], dcLayers[l+1], kerSize, kerSize, padSize, padSize))
    model:add(nn.ReLU())
    -- model:add(nn.Dropout(opt.dropout))
    model:add(nn.SpatialMaxPooling(poolSize, poolSize))
    image_size = image_size/poolSize;
