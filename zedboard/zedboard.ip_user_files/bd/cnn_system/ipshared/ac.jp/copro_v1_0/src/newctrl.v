...
module newctrl(/*AUTOARG*/
   // Outputs
   relu_oe, mem_bias_addr, buf_pix_en, breg_we, bias_oe, ack, wreg_we,
   conv_oe, pool_oe, buf_feat_en, mem_input_addr, mem_weight_addr,
   mem_feat_we, mem_feat_rst, mem_feat_addr, mem_feat_addr_d1,
   mem_output_we, mem_output_addr, w_img_size, w_fil_size, w_fea_size,
   w_pool_size,
   // Inputs
   pool_size, bias_addr, clk, xrst, req, input_addr, weight_addr,
   output_addr, total_in, total_out, img_size, fil_size
   );
`include "parameters.vh"
  ...
  input [BSIZE-1:0]	bias_addr;		// To ctrl_core of ctrl_core.v
  ...
  output		bias_oe;		// From ctrl_bias of ctrl_bias.v
  output		breg_we;		// From ctrl_core of ctrl_core.v
  output		buf_pix_en;		// From ctrl_core of ctrl_core.v
  output [BSIZE-1:0]	mem_bias_addr;		// From ctrl_core of ctrl_core.v
  output		relu_oe;		// From ctrl_relu of ctrl_relu.v
  // End of automatics
  output                ack;
  output                wreg_we;
  output                conv_oe;
